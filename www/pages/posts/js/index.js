...
async function getContent(){
    ...
    let git = await axios({ url: `https://raw.githubusercontent.com/kel-cu-infostructure/posts/main/files/${id}.md` })
    let info = git.data;
    ...
    if(data.hideContact) document.getElementById("contact").style = "display: none;";
    var converter = new showdown.Converter(),
        html = converter.makeHtml(info.replace(`${infoA[0]}\r\n`, ``).replace(`${infoA[0]}\n`, ``));
    document.getElementById("contentinner").innerHTML = html;
    document.getElementById("giturl").href = `https://github.com/kel-cu-infostructure/posts/blob/main/files/${id}.md`
    // console.log(infoA);
